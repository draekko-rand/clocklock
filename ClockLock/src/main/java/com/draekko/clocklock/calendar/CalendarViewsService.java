...
package com.draekko.clocklock.calendar;
...
import com.draekko.clocklock.calendar.CalendarInfo.EventInfo;
...
class CalendarRemoteViewsFactory implements RemoteViewsFactory {
    private static final String TAG = "CalRemViewsFactory";
    ...
    public RemoteViews getViewAt(int position) {
        ...
        if (Debug.doDebug(mContext)) {
            Log.v(TAG, "Showing at position " + position + " event: " + event.title);
        }
    ...
    private void updatePanelVisibility() {
        if (!mCalendarInfo.hasEvents()) {
            if (Debug.doDebug(mContext)) {
                Log.v(TAG, "No events - Hide calendar panel");
            }
            ...
            updatePlusIntent.setAction(ClockWidgetService.ACTION_HIDE_CALENDAR);
    ...
    private void getCalendarEvents(Context context, long lookahead, Set<String> calendars,
        ...
        if (cursor != null) {
            ...
            while (cursor.moveToNext() && eventCount < Constants.MAX_CALENDAR_ITEMS) {
                ...
                if (Debug.doDebug(mContext)) {
                    Log.v(TAG, "Adding event: " + title + " with id: " + eventId);
                }
    ...
    private long calculateUpdateTime(Context context) {
        ...
        if (Debug.doDebug(mContext)) {
            ...
            Log.i(TAG, "ClockLock: It is now " + DateFormat.getTimeFormat(context).format(date1)
